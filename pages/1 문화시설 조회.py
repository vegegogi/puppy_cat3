...
pet_place_clean = pd.read_csv('vegegogi\puppy_cat3\data\pet_place_clean_FN.csv').iloc[:, 1:]
...
for index, row in life[tmp_df].iterrows():
        ...
        folium.Marker([row['위도'], row['경도']], popup=popup_content, \
            icon=folium.Icon(color='pink', icon='music', prefix='fa')).add_to(map)
